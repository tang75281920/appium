...
|`--avd-args`|null|(Android-only) 要启动avd的添加额外的参数|`--avd-args -no-snapshot-load`|
...
|`--no-perms-check`|false|绕过Appium的检查,以确保我们可以读/写必要文件||
|`--command-timeout`|60|默认所有会话的服务的超时命令。 会被新的超时命令覆盖||
...
|`--isolate-sim-device`|false|Xcode 6存在一个bug，那就是一些平台上如果其他模拟器设备先被删除时某个特定的模拟器只能在没有任何错误的情况下被建立。这个选项导致了Appium不得不删除除了正在使用设备以外其他所有的设备。请注意这是永久性删除，你必须为使用simctl或xcode管理设备的类别负责，这些设备使用了Appium。||
|`--tmp`|null|可以用来管理临时文件的appium目录（绝对路径）,比如存放需要移动的内置iOS应用程序。 默认的变量为 `APPIUM_TMP_DIR` ，在 *nix/Mac 为 `/tmp` 在windows上为  `TEMP` 。||
|`--trace-dir`|null|用于保存ios instruments 痕迹的 appium 目录，是绝对路径, 默认为 <tmp dir>/appium-instruments||
|`--intent-action`|android.intent.action.MAIN|(Android-only) 用于启动 activity 的意图行动|`--intent-action android.intent.action.MAIN`|
